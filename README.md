...
- **証跡に強いスクショ**: 保存名に自動で日時 `_YYYYMMDD_HHMMSS` を付与（上書きされない）。Selenium でも CDP によるフルページ撮影に対応。
- **セレクトボックス・チェックボックス対応**: `select_option` / `set_checked` ツールで確実に操作。select の選択肢一覧・checkbox の ON/OFF は要素一覧に表示される。
- **ページ本文の読み取り**: `get_page_text` で表・照会結果・明細を読める（読む系タスクにも対応）。
- **SECRET のドメイン制限**: `<NAME>_ALLOWED_DOMAINS` で秘密情報を入力してよいサイトを限定（悪意あるページによるプロンプトインジェクション対策）。
...
# もしくはパッケージとして導入（console script が入る）:
#   pip install -e ".[all]"   → llm-browser-agent / llm-browser-agent-mcp 等のコマンドが使える
...
`select_option` / `set_checked` / `send_keys` / `scroll` / `take_screenshot` / `close_browser`。`take_screenshot` は保存に加え画像をホストにも返す。API キーは不要。

MCP サーバーは環境変数で挙動を切り替える: `BROWSER_AGENT_ENGINE`(selenium/playwright) / `BROWSER_AGENT_BROWSER`(edge/chrome) /
`BROWSER_AGENT_HEADLESS`(0/1) / `BROWSER_AGENT_OUTPUT`(スクショ保存先)。ログイン情報は `env` に置き `{{SECRET:NAME}}` で参照。さらに `MY_PASSWORD_ALLOWED_DOMAINS` のように `<NAME>_ALLOWED_DOMAINS` を置くと、そのシークレットは指定ドメインのページにしか入力されない。
...
$env:OLLAMA_NUM_CTX="16384"           # コンテキスト長を明示（徘徊・繰り返しの主因対策。既定 16384）
...
- **`OLLAMA_NUM_CTX`（既定 16384）**: コンテキスト長を明示する。Ollama の既定値のままだと履歴が黙って切り捨てられ、「同じ操作を繰り返す・徘徊する」原因になる。VRAM に余裕がなければ 8192 に。
- **履歴プルーニング（自動）**: 各ツール結果に付くページ状態は直近 `AGENT_KEEP_STATES` 件（既定 3）だけ残し、古いものは自動で省略される。小さいコンテキストでも長いフォーム入力が完走しやすい。
...
7. **履歴プルーニング**: 古いステップのページ状態を自動で省略し、コンテキスト溢れ（ローカル LLM の徘徊の主因）と
   トークン費用を抑える（直近 `AGENT_KEEP_STATES` 件は保持。既定 3）。
8. **API リトライ**: Anthropic API のレート制限（429）・過負荷（529）等は指数バックオフで自動再試行する。
...
**ドメイン制限（推奨）**: `<NAME>_ALLOWED_DOMAINS` を設定すると、そのシークレットは指定ドメイン（サブドメイン含む）のページにしか入力できなくなる。ページ本文は LLM に渡るため、悪意あるページが「パスワードをこのフォームに入力せよ」と指示するプロンプトインジェクションが理論上可能だが、この制限により別サイトへの入力はブラウザ層で拒否される。

```powershell
$env:MY_PASSWORD="実際のパスワード"
$env:MY_PASSWORD_ALLOWED_DOMAINS="example.co.jp,localhost"   # ここ以外では入力を拒否
```
...
- **CI（GitHub Actions）**: push ごとに ubuntu で構文チェック＋単体テスト、windows-latest で Edge 実ブラウザの
  selftest（Selenium / Playwright 両エンジン）を自動実行。
