...
### 注文受諾・納入登録の起点は「仕入先ホーム」

ログイン直後の画面から、同じ道を通って起点に着く。
...
利用者は**ログインするだけ**でよい。帳票の発行と取得も同じ道を通り、
そこから「帳票出力」へ進む。
...
```
帳票出力 → ステータス・発注番号(From/To)・納入日(From/To) を入力 → 続行(C)
...
```
帳票出力 → 要求の表示 → 要求要約表から要求 ID の行を探して出力 → Excel
```
