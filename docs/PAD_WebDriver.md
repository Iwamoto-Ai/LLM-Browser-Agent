...
**運用**　[🔐 資格情報の扱い](#-資格情報の扱い) / [手動ログイン](#手動ログイン) / [📊 結果 CSV と再実行](#-結果-csv-と再実行) / [🔗 登録した結果を次のバッチへ渡す](#-登録した結果を次のバッチへ渡す) / [📥 エビデンスがファイルで届く場合](#-エビデンスがファイルで届く場合) / [📸 エビデンス（スクリーンショット）](#-エビデンススクリーンショット)
...
| ダウンロード | チェックすると、その操作で落ちてくるファイルを保存する |
| 値を記録 | チェックすると、**押す代わりにその場所の文字を読んで**結果 CSV の列にする |
...
納入登録で要求 ID が出て、しばらく経ってから検収状況一覧を取りに行く、
というような形。

`capture` で画面の値を読み取ると、**結果 CSV の列として残る。**

```json
{"type": "capture", "selectors": [["#ShipDoneNo"]], "name": "要求ID"}
```

振り分け画面では**「値を記録」にチェックを入れて列名を書く。** チェックした行は
...
> **チェックを入れ忘れると、元の明細 CSV を読んだまま動く。** 要求 ID の列が
> あれば処理は進んでしまい、**中身の合わないエビデンスが集まる**ので気づきにくい。
> ②を作るときは必ず確かめること。

> **実サイトでは発行から出力まで 1 分〜3 時間かかる。** ①と②を続けて流さず、
> 時間をおいて②を実行する。結果 CSV が残っているので、いつでも再開できる。
...
> **仕組み。** ウィンドウを広げてから撮る方法もありますが、**高さは物理画面
> までしか広げられません**（実機で 6000 を要求して 1220 が上限でした）。
> そこで Chrome / Edge 独自の命令を使っています。使えないブラウザーでは
> 自動的に見えている範囲だけの撮影に落ちるので、止まることはありません。
