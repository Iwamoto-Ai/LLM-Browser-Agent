...
「この振り分けでバッチ定義を作る」を押すと下の欄に結果が出る。**そのまま
...
| 箱 | 意味 | 何回動くか | 入れるもの |
| --- | --- | --- | --- |
| `setup` | **最初に 1 回だけやること** | 1 回 | ページを開く、ログイン、繰り返しの起点画面まで進む |
...
- **手動ログイン** … 人が手でログインし、起点画面まで進める。PAD はパスワードを
...
ログインは人が画面で行い、繰り返しの起点画面まで進めてから[OK]を押す。
...
ログイン操作も、起点画面までのメニュー移動も、人がやる前提なので生成しない。
...
ログイン後の画面構成はサイト側の都合で変わることがある。機械的に辿らせるより
...
セットアップ失敗で止まる。実サイトでまさにこれが起きた。人が起点まで進めばその差異に影響されない。
そのため生成器は、録画のログイン操作も起点までの移動も再生しない。
...
なぜ「見分けて除く」のではなく「読まない」設計にしたかは
[資格情報を「読まない」設計](PAD_WebDriver_internals.md#-資格情報を読まない設計) を参照。
