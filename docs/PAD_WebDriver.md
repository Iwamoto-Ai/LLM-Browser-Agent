...
- ウィンドウの位置がずれても動く
- 画面の解像度が違う PC でも動く
- 画面を見ていなくても動くので、自動実行中に他の業務ができます。
  普段使っているブラウザーや他の業務アプリは自由に使って大丈夫です。
  ただし自動実行中のブラウザーは閉じたり最小化したりはしないでください。
- 自動実行しているウインドウの上に別ウインドウがかぶさっても影響しない。
...
チェックを入れると、**列名・キー・数字だけ**の 3 つが出る。

| 欄 | 入れるもの |
| --- | --- |
| 列名 | 結果 CSV の列の名前（例: `要求ID`） |
| キー | 画面でこの文字を探し、**その直後を記録する**（例: `要求IDは`）。空なら、その行の場所の文字を読む |
| 数字だけ | キーの直後の**最初の数字のまとまり**だけを取る |
...
**キーに `要求IDは` と入れ、「数字だけ」にチェック**すると `131982564` になる。
次のバッチにそのまま渡せる。

```json
{"type": "capture", "key": "要求IDは", "extract": "digits", "name": "要求ID"}
```

**キーを使うと、要素を指定しなくてよい。** 画面全体（iframe の中も）から探すので、
ポップアップのように要素の見分けがつきにくいものでも、見えている文字で指せる。

| | キーの直後から | 終わり |
| --- | --- | --- |
| 数字だけ | 最初の数字（間の空白やコロンは飛ばす） | 数字が途切れたところ |
| そのまま | 最初の文字（間の空白やコロンは飛ばす） | 空白・改行・「。」「、」のどれか（最大 50 字） |

> **キーは、画面に 1 回しか出ない形にする。** 同じ文字が複数あると最初のものを取る。
> `要求ID` だけだと一覧の見出しにも出るので、`要求IDは` のように文中にしか
> 出ない形で書く。

キーを空にして「数字だけ」を付けたときは、従来どおり**その行の場所の文字**から
数字を全部つなげて取る。同じ場所に他の数字があると混ざるので、キーを使うほうが確実。

#### スクロールしないと出てこない一覧

一覧に検索欄が無く、**スクロールして初めて次の行が読み込まれる**画面がある。

```json
{"type": "scrollFind", "text": "{{要求ID}}", "tries": 10}
```

見つかるまで少しずつ下げながら探す。`tries` は下げる回数（既定 20）。
見つかったら、そのまま `row/` でその行のリンクを押せる。

> **遡れる範囲には限りがある。** 新しい順に並ぶ一覧で、他の人の分も積まれる
> ような画面では、時間が経つと目的の行が下へ流れて現実的に届かなくなる
> （実 EDI では 5 時間で数百件）。**記録した番号を使う処理は、間を空けずに
> 流すのが前提。**
...
## ⚠️ 制約
- 自動実行中のブラウザーと PAD は、手動ログインやダイアログの[OK]など
  必要な操作以外では触らない
- 自動実行中のブラウザーは閉じたり最小化したりしない（最小化すると途中で止まる）
- 別のサイトから読み込まれた iframe の中には届かない
