...
**実機で分かったこと**　[⚠️ Web.InvokeWebService の引数（最重要）](#-webinvokewebservice-の引数最重要) / [🩺 WebDriver の応答を先に見る](#-webdriver-の応答を先に見る) / [📷 ページ全体を 1 枚に撮る](#-ページ全体を-1-枚に撮る) / [🧪 バッチ定義を練習サイトで再生する](#-バッチ定義を練習サイトで再生する) / [📌 Robin リテラルのエスケープ（実機で判明）](#-robin-リテラルのエスケープ実機で判明) / [✅ 実機で確認できたアクション書式（PAD 無料版 / Windows 11）](#-実機で確認できたアクション書式pad-無料版--windows-11)
...
`tools/replay_practice.mjs` は、練習用のバッチ定義を練習サイトに対して実際に
再生する。**要素の探し方は生成物と同じ共通 JavaScript を使う**ので、PAD で
動かしたときと同じ結果になる。

```
node tools/replay_practice.mjs
OK  recordings/edi2_accept_batch.json → 27 ステップ通過
```

バッチ定義と練習サイトは別々に直すため、片方だけ変えると噛み合わなくなる。
実際、練習サイトの導線を実 EDI に合わせたとき、ループ途中に残っていた
古い戻り方に気づけず「ステップ11（click #LI取引情報入力・照会）で要素が
見つかりません」で落ちた。**このテストがあれば貼り付ける前に分かる。**

ステップ番号は生成物と同じ数え方（`setup` と `loop` で別々に 1 から）にして
...
W3C の `/screenshot` は**見えている範囲だけ**を返す。縦に長い画面では、
スクロールしないと読めないエビデンスになる。

### ウィンドウを広げる方法は使えない

高さは**物理画面までしか広げられない。** 実機で測った結果:

```
3000 x 6000 を要求 → 幅=3000（通る）  高さ=1220（頭打ち）
```

幅は画面外にはみ出せるが、高さは Windows の制約で止まる。横長には使えるが、
縦長には足りない。

### Chrome / Edge 独自の命令を使う

W3C 標準の外に、DevTools の命令を直接送る口がある。

```
POST /session/{id}/chromium/send_command_and_get_result
     {"cmd": "Page.captureScreenshot",
      "params": {"format": "png", "captureBeyondViewport": true}}
```

`captureBeyondViewport` が「見えていない部分まで含めて撮る」指定。
**実機（Edge）で確認済み。** Chrome も同じ Chromium なので同様に動く。

応答の形が標準と違う点に注意。

| | base64 の位置 |
| --- | --- |
| W3C `/screenshot` | `value` |
| CDP `captureScreenshot` | `value.data` |

生成器は `FullShot` が `True` ならこちらを使い、HTTP が 200 以外なら
従来の `/screenshot` に落とす。**使えない環境でも止まらない。**
